...
export function useAStar() {
  ...
  const runAStar = useCallback(async (startStarId: number, destinationStarId: number, fuel: number) => {
    try {
      setLoading(true);
      setError(null);
      ...
      const fullUrl = `${CONFIG.ASTAR_URL}?${params.toString()}`;
  ...
  return { pathStarIds, pathSequence, loading, error, pathDistance, runAStar };
}
